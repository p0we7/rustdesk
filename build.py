...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libliblidesk.dylib target/release/liblidesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/LiDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"LiDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon LiDesk.app 200 190 --hide-extension LiDesk.app lidesk.dmg ./build/macos/Build/Products/Release/LiDesk.app")
    os.rename("lidesk.dmg", f"../lidesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/liblidesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/lidesk.exe')
    os.chdir('../..')
